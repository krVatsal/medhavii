      ...
      <>
        ...
        <div className="relative z-10 grid grid-cols-[600px_1fr] gap-[64px] px-[80px] pt-[12px]">
                ...
                {cards.map((item, idx) => (
                  <li key={idx} className="flex items-start gap-[24px]">
                    ...
                    <div className="flex-1">
                      <TextWithLaTeX as="h3" className=" text-[24px] font-semibold leading-[1.15]" style={{ color: 'var(--text-heading-color, #111827)' }} content={item.title} />
                      <TextWithLaTeX as="p" className=" text-[16px] leading-[1.55] mt-[8px]" style={{ color: 'var(--text-body-color, #6B7280)' }} content={item.description} />
                    </div>
                  </li>
                ))}
              </ul>
          ...
          <div className="pt-[28px]">
            <TextWithLaTeX as="h1" className="font-['Playfair Display'] text-[64px] leading-[1.05] tracking-[-0.01em] max-w-[600px] font-semibold" style={{ color: 'var(--text-heading-color, #111827)' }} content={ (slideData?.rightTitle || "Disrupting the\nIndustry").replace(/\n/g, "<br/>") }  />
            <div className="mt-[24px] inline-block rounded-md px-6 py-4" style={{ backgroundColor: 'var(--secondary-accent-color, #F3F4F6)' }}>
              <TextWithLaTeX as="p" className="font-['Playfair Display'] text-[16px] leading-[1.6] max-w-[620px]" style={{ color: 'var(--text-body-color, #6B7280)' }} content={slideData?.rightDescription || "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna"} />
            </div>

...
<p align="center">
  ...
  &nbsp;
  <a href="https://x.com/medhaviai">
    <img src="https://img.shields.io/badge/X-Follow%20Us-000000?logo=twitter&style=for-the-badge" alt="Follow us on X" />
  </a>
</p>

# मेधावी (Medhavi) - Open-Source AI Presentation Generator for India

**Medhavi** (meaning "intelligent" or "wise" in Sanskrit) is an open-source Indian AI presentation generator with native multilingual support, regional context awareness, and voice narration in 11+ Indian languages — all running locally on your device. Stay in control of your data and privacy while using models like OpenAI and Gemini, or use your own hosted models through Ollama.

__✨ Now with Voice Narration in Hindi, Tamil, Telugu, Bengali, Marathi, Gujarati, Kannada, Malayalam, Punjabi, Odia, and English using Bhashini API!__

__✨ Generate presentations with regional context - examples and references familiar to your audience!__

__✨ Create presentations from your existing PPTX files! Upload your presentation to create template designs and generate on-brand presentations on any topic.__
...
> For enterprise use, custom deployments, or partnership opportunities, contact us at **[hello@medhavi.ai](mailto:hello@medhavi.ai)**.

> [!IMPORTANT]
> Like Medhavi? A ⭐ star shows your support and encourages us to keep building!

> [!TIP]
> For detailed setup guides, API documentation, and advanced configuration options, visit our **[Official Documentation](https://docs.medhavi.ai)**


## ✨ भारत के लिए AI Presentations (AI Presentations for India)

Medhavi gives you complete control over your AI presentation workflow with Indian context. Choose your models, customize with regional references, add voice narration in Indian languages, and keep your data private.

* ✅ **Multilingual Support** — Create presentations in Hindi, Tamil, Telugu, Bengali, Marathi, Gujarati, Kannada, Malayalam, Punjabi, Odia, English
* ✅ **Voice Narration (NEW!)** — Teaching-style audio explanations using Bhashini API in 11+ Indian languages
* ✅ **Regional Context** — Examples and references familiar to Indian audiences (Delhi Metro, Mumbai Dabbawalas, Chennai IT Corridor, etc.)
...
## Medhavi Cloud
<a href="https://medhavi.ai" target="_blank" align="center">
  
  <img src="readme_assets/cloud-banner.png" height="350" alt="Medhavi Logo" />
</a>

## Running Medhavi Docker

#### 1. Start Medhavi

##### Linux/MacOS (Bash/Zsh Shell):
```bash
docker run -it --name medhavi -p 5000:80 -v "./app_data:/app_data" ghcr.io/medhavi/medhavi:latest
```

##### Windows (PowerShell):
```bash
docker run -it --name medhavi -p 5000:80 -v "${PWD}\app_data:/app_data" ghcr.io/medhavi/medhavi:latest
```

#### 2. Open Medhavi
Open http://localhost:5000 on browser of your choice to use Medhavi.
...
### Using OpenAI
```bash
docker run -it --name medhavi -p 5000:80 -e LLM="openai" -e OPENAI_API_KEY="******" -e IMAGE_PROVIDER="dall-e-3" -e CAN_CHANGE_KEYS="false" -v "./app_data:/app_data" ghcr.io/medhavi/medhavi:latest
```

### Using Google
```bash
docker run -it --name medhavi -p 5000:80 -e LLM="google" -e GOOGLE_API_KEY="******" -e IMAGE_PROVIDER="gemini_flash" -e CAN_CHANGE_KEYS="false" -v "./app_data:/app_data" ghcr.io/medhavi/medhavi:latest
```

### Using Ollama
```bash
docker run -it --name medhavi -p 5000:80 -e LLM="ollama" -e OLLAMA_MODEL="llama3.2:3b" -e IMAGE_PROVIDER="pexels" -e PEXELS_API_KEY="*******" -e CAN_CHANGE_KEYS="false" -v "./app_data:/app_data" ghcr.io/medhavi/medhavi:latest
```

### Using Anthropic
```bash
docker run -it --name medhavi -p 5000:80 -e LLM="anthropic" -e ANTHROPIC_API_KEY="******" -e IMAGE_PROVIDER="pexels" -e PEXELS_API_KEY="******" -e CAN_CHANGE_KEYS="false" -v "./app_data:/app_data" ghcr.io/medhavi/medhavi:latest
```

### Using OpenAI Compatible API
```bash
docker run -it -p 5000:80 -e CAN_CHANGE_KEYS="false"  -e LLM="custom" -e CUSTOM_LLM_URL="http://*****" -e CUSTOM_LLM_API_KEY="*****" -e CUSTOM_MODEL="llama3.2:3b" -e IMAGE_PROVIDER="pexels" -e  PEXELS_API_KEY="********" -v "./app_data:/app_data" ghcr.io/medhavi/medhavi:latest
```

#### Running Medhavi with GPU Support
...
Once the NVIDIA Container Toolkit is installed and configured, you can run Medhavi with GPU support by adding the `--gpus=all` flag:

```bash
docker run -it --name medhavi --gpus=all -p 5000:80 -e LLM="ollama" -e OLLAMA_MODEL="llama3.2:3b" -e IMAGE_PROVIDER="pexels" -e PEXELS_API_KEY="*******" -e CAN_CHANGE_KEYS="false" -v "./app_data:/app_data" ghcr.io/medhavi/medhavi:latest
```
...
For detailed info checkout [API documentation](https://docs.medhavi.ai/using-medhavi-api).

### API Tutorials
- [Generate Presentations via API in 5 minutes](https://docs.medhavi.ai/tutorial/generate-presentation-over-api)
- [Create Presentations from CSV using AI](https://docs.medhavi.ai/tutorial/generate-presentation-from-csv)
- [Create Data Reports Using AI](https://docs.medhavi.ai/tutorial/create-data-reports-using-ai)
- [Add Voice Narration in Regional Languages](https://docs.medhavi.ai/tutorial/voice-narration-api)

## Roadmap
- [x] Support for custom HTML templates by developers
- [x] Support for accessing custom templates over API
- [x] Implement MCP server
- [x] Voice narration in 11+ Indian languages (Bhashini integration)
...
### 1. Configure your AI models and API keys
![Demo](readme_assets/images/prompting.png)

### 2. Add prompt, select number of slides, language, and regional preferences
![Demo](readme_assets/images/select-theme.png)

### 3. Review and edit outline with AI suggestions
![Demo](readme_assets/images/outline.png)

### 4. Select theme that matches your brand
![Demo](readme_assets/images/select-theme.png)

### 5. Present with voice narration in your preferred language
![Demo](readme_assets/images/present.png)

### 6. Change theme anytime to refresh the look
...
- [Voice Narration Guide](VOICE_NARRATION_GUIDE.md) - Complete guide to voice narration feature
- [Regional References Guide](REGIONAL_REFERENCES_GUIDE.md) - How to use regional context effectively
- [AI Pipeline Documentation](AI_PIPELINE_DETAILED_PROMPT.md) - Detailed AI workflow documentation
- [API Documentation](https://docs.medhavi.ai) - Complete API reference

## Community & Support
[Discord](https://discord.gg/9ZsKKxudNE) | [Twitter](https://x.com/medhaviai) | [Email](mailto:hello@medhavi.ai)
